...
class ShoppingProductController extends Controller
{
    ...
    public function ShoppingSliderList()
    {
        ...
        ->addColumn('slider_image', function($row){
            if($row->slider_image){
                $slider_image = '<img src="'.asset("shopping/slider/thumb/".$row->slider_image).'" width="100"/>';
            }
            return $slider_image;
        })
    ...
    public function ShoppingProductList(){
        ...
        ->addColumn('main_image', function($row){
            if($row->main_image){
                $main_image = '<img src="'.asset("shopping/product/thumb/".$row->main_image).'" width="50"/>';
            }
            return $main_image;
        })
        ->addColumn('action', function($row){
    ...
    function sliderImageInsert($image, Request $request, $flag){
        $destination = base_path().'/public/shopping/slider/';
        $image_extension = $image->getClientOriginalExtension();
        $image_name = md5(date('now').time()).$flag.".".$image_extension;
        $original_path = $destination.$image_name;
        Image::make($image)->save($original_path);
        $thumb_path = base_path().'/public/shopping/slider/thumb/'.$image_name;
        Image::make($image)
        ->resize(300, 400)
        ->save($thumb_path);
